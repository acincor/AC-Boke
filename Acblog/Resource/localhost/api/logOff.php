    ...
    if(!is_bool($query)) {
            ...
            while($arr = mysqli_fetch_assoc($file)){
                    ...
                    if(!is_bool($pic_urls)){
                        ...
                        for ($i = 0; $i < count($pic_urls); $i++) {
                             unlink(".".explode("http://localhost:8080/api",$pic_urls[$i]['pic'.$i])[1]);
                             unlink(".".explode("thumbnail",explode("http://localhost:8080/api",$pic_urls[$i]['pic'.$i])[1])[0].explode("thumbnail",explode("http://localhost:8080/api",$pic_urls[$i]['pic'.$i])[1])[1]);
                        }
                    }
        ...
        $select_bool = mysqli_query($mysql,"SELECT * FROM users WHERE uid = ".$array['uid']);
            if(!is_bool($select_bool)) {
                ...
                if($arr != NULL) {
                    if(file_exists(".".explode("http://localhost:8080/api",$arr['portrait'])[1])) {
                        unlink(".".explode("http://localhost:8080/api",$arr['portrait'])[1]);
                    }
                    if(is_dir("./".$array["uid"]."/portrait")) {
                        rmdir("./".$array["uid"]."/portrait");
                    }
                    if(is_dir("./".$array["uid"]."/thumbnail")) {
                        rmdir("./".$array["uid"]."/thumbnail");
                    }
                    if(is_dir("./".$array["uid"])) {
                        rmdir("./".$array["uid"]);
                    }
