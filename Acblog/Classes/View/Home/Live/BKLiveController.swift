...
class BKLiveController: UIViewController {
    ...
    @objc func startLive(_ sender: Any) {
        ...
        self.stream.attachAudio(AVCaptureDevice.default(for: .audio)) { _,error  in
            //print(error)
        }
        
        self.stream.attachCamera(AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: .back), track: 0) { _, error in
            if let error {
                print(error)
            }
        }
