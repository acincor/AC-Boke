    ...
    if(!is_bool($query)) {
        ...
        if($array != NULL) {
            ...
            if(isset($_GET['since_id'])) {
                $query = mysqli_query($mysql,"select createTime from blogs where id = ".$_GET['since_id']);
                if(!is_bool($query)) {
                    $array = mysqli_fetch_assoc($query);
                    if($array != NULL) {
                        $createTime = $array["createTime"];
                        $query = mysqli_query($mysql,"select * from blogs where createTime > '".$createTime."'"." ORDER BY createTime DESC");
                    }
                }
            } else if(isset($_GET['max_id'])) {
                $query = mysqli_query($mysql,"select createTime from blogs where id = ".$_GET['max_id']);
                if(!is_bool($query)) {
                    $array = mysqli_fetch_assoc($query);
                    if($array != NULL) {
                        $createTime = $array["createTime"];
