...
override var preferredContainerBackgroundStyle: UIContainerBackgroundStyle {
    ...
    @objc func loadData() {
        ...
        commentListViewModel.loadStatus(uid) { (isSuccessed) in
            ...
            if !isSuccessed {
                SVProgressHUD.showInfo(withStatus: NSLocalizedString("暂无评论任何博客哦！", comment: ""))
    ...
    @objc func action(sender: UITapGestureRecognizer) {
        ...
        present(UIHostingController(rootView: UserNavigationLinkView(account: uvm, uid: sender.sender)), animated: true)
    }
...
extension CommentStatusTableViewController {
    ...
    @objc func action1(_ sender: UIButton) {
        sender.identifier.bottomView.deleteBlog(commentListViewModel.statusList[sender.tag].status.id) { Result, Error in
            ...
            if let i = listViewModel.statusList.firstIndex(where: { vm in
                vm.status.id == commentListViewModel.statusList[sender.tag].status.id
            }){
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        viewModel = commentListViewModel.statusList[indexPath.row]
