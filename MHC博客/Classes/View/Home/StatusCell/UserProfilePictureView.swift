...
let UserProfilePictureViewItemMargin: CGFloat = 8
let UserProfilePictureCellId = "UserProfilePictureCellId"
class UserProfilePictureView:UICollectionView {
    var viewModel: URL
    init(viewModel: URL) {
...
extension UserProfilePictureView: UICollectionViewDataSource {
...
class UserProfilePictureViewCell: UICollectionViewCell {
    var imageURL: URL? {
        didSet {
...
extension UserProfilePictureView:UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        //print("单击照片\(indexPath) \(viewModel?.thumbnailUrls)")
        let userInfo = [WBUserProfileSelectedPhotoIndexPathKey: indexPath, WBUserProfileSelectedPhotoURLsKey: [viewModel]] as [String : Any]
        NotificationCenter.default.post(name: Notification.Name(WBUserProfileSelectedPhotoNotification), object: self, userInfo: userInfo)
        //photoBrowserPresentFromRect(indexPath: indexPath)
        photoBrowserPresentToRect(indexPath: indexPath)
    }
}
extension UserProfilePictureView: ProfileBrowserPresentDelegate{
